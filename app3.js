...
function scheduleSignals() {
    ...
    signalHours.forEach((hour) => {
        ...
        const timeInLocal = moment
            .tz({ hour: hour, minute: randomMinute }, "America/Sao_Paulo")
            .local();

        console.log(`Agendado para ${timeInLocal.format('HH:mm')} BRT.`);
        schedule.scheduleJob(
            ...
            function () {
                console.log(`Enviando sinal para ${hour}:${randomMinute} BRT.`);
                sendSignal(GROUP_ID);
            }
        );
    });
